...
dross::DirectionalEdgeFacts open_edge(const std::uint32_t cost = 1) {
  return dross::DirectionalEdgeFacts{
      .traversable = true, .cost = dross::MovementCost{cost}};
}

dross::DirectionalEdgeFacts closed_edge() {
  return dross::DirectionalEdgeFacts{
      .traversable = false, .cost = dross::MovementCost{1}};
}
...
dross::CompiledHexMap build_diamond(const bool reverse_insertion,
  ...
  const std::vector cells{path_cell(0, 0), path_cell(0, 1), path_cell(1, 0),
                          path_cell(1, 1)};
  ...
  REQUIRE(builder.add_edge(cells[0], cells[1], open_edge(lower_cost),
                           open_edge(lower_cost)));
  REQUIRE(builder.add_edge(cells[1], cells[3], open_edge(lower_cost),
                           open_edge(lower_cost)));
  REQUIRE(builder.add_edge(cells[0], cells[2], open_edge(upper_cost),
                           open_edge(upper_cost)));
  REQUIRE(builder.add_edge(cells[2], cells[3], open_edge(upper_cost),
                           open_edge(upper_cost)));
...
TEST_CASE("traversal assessment reports blocked cells, edges, and occupancy") {
  ...
  REQUIRE(builder.add_edge(path_cell(0, 0), path_cell(1, 0), closed_edge(),
                           open_edge()));
  REQUIRE(builder.add_edge(path_cell(0, 0), path_cell(0, 1), open_edge(),
                           open_edge()));
  ...
  assessment = dross::assess_transition(
      map, occupancy, footprint,
      dross::HexPose{path_cell(0, 0), dross::HexFacing::east},
  ...
  assessment = dross::assess_placement(
      map, occupancy, footprint,
      dross::HexPose{path_cell(1, 0), dross::HexFacing::east}, actor);
...
TEST_CASE("planner tie-breaking is independent of map insertion order") {
  ...
  const auto first =
      planner.plan(first_map, occupancy, footprint, start, goal, policy,
                   dross::EntityId{1, 1});
  const auto second =
      planner.plan(second_map, occupancy, footprint, start, goal, policy,
                   dross::EntityId{1, 1});
  REQUIRE(first);
  REQUIRE(second);
...
TEST_CASE("asymmetric footprint rotates to traverse a narrow route") {
  dross::CompiledHexMapBuilder builder;
  for (const auto& cell :
       {path_cell(0, 0), path_cell(1, 0), path_cell(0, 1), path_cell(0, 2)}) {
    REQUIRE(builder.add_cell(path_facts(cell)));
  }
  REQUIRE(builder.add_edge(path_cell(0, 0), path_cell(0, 1), open_edge(),
                           open_edge()));
  REQUIRE(builder.add_edge(path_cell(0, 1), path_cell(0, 2), open_edge(),
                           open_edge()));
  const auto map = std::move(builder).build().value();
  const auto footprint =
      dross::FootprintDefinition::create(
          dross::FootprintId{
              dross::ContentId::parse("demo:asymmetric_two").value()},
